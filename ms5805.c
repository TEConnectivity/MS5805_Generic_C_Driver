 ...
 * \file ms5805.c
 *
 * \brief MS5805 Temperature sensor driver source file
 *
 * Copyright (c) 2016 Measurement Specialties. All rights reserved.
 *
 * For details on programming, refer to ms5805 datasheet :
...
#define MS5805_CONVERSION_TIME_OSR_256								1000
#define MS5805_CONVERSION_TIME_OSR_512								2000
#define MS5805_CONVERSION_TIME_OSR_1024								3000
#define MS5805_CONVERSION_TIME_OSR_2048								5000
#define MS5805_CONVERSION_TIME_OSR_4096								9000
#define MS5805_CONVERSION_TIME_OSR_8192								17000
...
#define MS5805_COEFFICIENT_NUMBERS									7

// Static functions
static enum ms5805_status ms5805_write_command(uint8_t);
static enum ms5805_status ms5805_read_eeprom_coeff(uint8_t, uint16_t*);
static enum ms5805_status ms5805_read_eeprom(void);
static enum ms5805_status ms5805_conversion_and_read_adc( uint8_t, uint32_t *);
static bool ms5805_crc_check (uint16_t *n_prom, uint8_t crc);

enum ms5805_resolution_osr ms5805_resolution_osr;
static uint16_t eeprom_coeff[MS5805_COEFFICIENT_NUMBERS+1];
static uint32_t conversion_time[6] = {	MS5805_CONVERSION_TIME_OSR_256,
										MS5805_CONVERSION_TIME_OSR_512,
										MS5805_CONVERSION_TIME_OSR_1024,
										MS5805_CONVERSION_TIME_OSR_2048,
										MS5805_CONVERSION_TIME_OSR_4096,
										MS5805_CONVERSION_TIME_OSR_8192};

// Default value to ensure coefficients are read before converting temperature
bool ms5805_coeff_read = false;
...
{
	ms5805_resolution_osr = ms5805_resolution_osr_8192;
...
{
	return ms5805_write_command(MS5805_RESET_COMMAND);
}
 ...
 *
 */
void ms5805_set_resolution(enum ms5805_resolution_osr res)
{
 ...
 *       - ms5805_status_no_i2c_acknowledge : I2C did not acknowledge
 */
enum ms5805_status ms5805_write_command( uint8_t cmd)
{
	enum status_code i2c_status;
	uint8_t data[1];
 ...
 */
enum ms5805_status ms5805_read_eeprom_coeff(uint8_t command, uint16_t *coeff)
{
	...
	status = ms5805_write_command(command);
...
enum ms5805_status ms5805_read_eeprom(void)
{
	...
	for( i=0 ; i< MS5805_COEFFICIENT_NUMBERS ; i++)
	{
		status = ms5805_read_eeprom_coeff( MS5805_PROM_ADDRESS_READ_ADDRESS_0 + i*2, eeprom_coeff+i);
		if(status != ms5805_status_ok)
			return status;
	}
    
	if( !ms5805_crc_check( eeprom_coeff, (eeprom_coeff[MS5805_CRC_INDEX] & 0xF000)>>12 ) )
		return ms5805_status_crc_error;
	
	ms5805_coeff_read = true;
 ...
 *       - ms5805_status_no_i2c_acknowledge : I2C did not acknowledge
 */
static enum ms5805_status ms5805_conversion_and_read_adc(uint8_t cmd, uint32_t *adc)
{
	enum ms5805_status status;
	enum status_code i2c_status;
	uint8_t buffer[3];
	...
	status = ms5805_write_command(cmd);
	// delay conversion depending on resolution
	delay_ms( conversion_time[ (cmd & MS5805_CONVERSION_OSR_MASK)/2 ]/1000 );
	if( status != ms5805_status_ok)
		return status;
 ...
 *       - ms5805_status_crc_error : CRC check error on the coefficients
 */
enum ms5805_status ms5805_read_temperature_and_pressure( float *temperature, float *pressure)
{
	...
	if( ms5805_coeff_read == false )
		status = ms5805_read_eeprom();
	if( status != ms5805_status_ok)
		return status;
	...
	status = ms5805_conversion_and_read_adc( cmd, &adc_temperature);
	if( status != ms5805_status_ok)
		return status;
	...
	status = ms5805_conversion_and_read_adc( cmd, &adc_pressure);
	if( status != ms5805_status_ok)
		return status;
	...
	dT = (int32_t)adc_temperature - ((int32_t)eeprom_coeff[MS5805_REFERENCE_TEMPERATURE_INDEX] <<8 );
	
	// Actual temperature = 2000 + dT * TEMPSENS
	TEMP = 2000 + ((int64_t)dT * (int64_t)eeprom_coeff[MS5805_TEMP_COEFF_OF_TEMPERATURE_INDEX] >> 23) ;
	...
	// OFF = OFF_T1 + TCO * dT
	OFF = ( (int64_t)(eeprom_coeff[MS5805_PRESSURE_OFFSET_INDEX]) << 17 ) + ( ( (int64_t)(eeprom_coeff[MS5805_TEMP_COEFF_OF_PRESSURE_OFFSET_INDEX]) * dT ) >> 6 ) ;
	OFF -= OFF2 ;
	
	// Sensitivity at actual temperature = SENS_T1 + TCS * dT
	SENS = ( (int64_t)eeprom_coeff[MS5805_PRESSURE_SENSITIVITY_INDEX] << 16 ) + ( ((int64_t)eeprom_coeff[MS5805_TEMP_COEFF_OF_PRESSURE_SENSITIVITY_INDEX] * dT) >> 7 ) ;
	SENS -= SENS2 ;
 ...
 */
bool ms5805_crc_check (uint16_t *n_prom, uint8_t crc)
{
	uint8_t cnt, n_bit;
	...
	n_rem = 0x00;
	crc_read = n_prom[0];
	n_prom[MS5805_COEFFICIENT_NUMBERS] = 0;
	n_prom[0] = (0x0FFF & (n_prom[0]));    // Clear the CRC byte

	for( cnt = 0 ; cnt < (MS5805_COEFFICIENT_NUMBERS+1)*2 ; cnt++ ) {
